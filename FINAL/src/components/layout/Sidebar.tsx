  ...
  return (
    <aside className="w-64 bg-sidebar text-sidebar-foreground flex flex-col min-h-screen border-r border-white/5 z-50">
      <div className="p-6 border-b border-white/5">
        <h1 className="text-2xl font-bold tracking-tight text-white italic">Slideverse<span className="text-primary neon-text">Pro</span></h1>
      </div>
      <nav className="flex-1 px-4 space-y-2 mt-4 overflow-y-auto">
        <Link
          ...
          className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${
            pathname === "/" 
              ? "bg-primary/10 text-primary font-bold shadow-[0_0_15px_rgba(212,255,0,0.2)] border border-primary/20" 
            ...
            <div className="mt-2 ml-4 pl-4 border-l border-zinc-800 space-y-1">
              {categories.map((cat) => (
                <Link
                  ...
                  title={cat.title}
                >
                  {typeof cat.title === 'object' ? (cat.title[language] || cat.title.en) : cat.title}
                </Link>
              ))}
            </div>
          )}
