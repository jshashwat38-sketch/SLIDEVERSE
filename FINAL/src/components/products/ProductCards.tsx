...
export function ProductCard({ id, title, description, price, features, imageUrl, images }: ProductProps) {
  ...
  return (
    <motion.div 
      ...
      className="group bg-[#09090B] border border-white/5 rounded-[2.5rem] overflow-hidden flex flex-col h-full hover:border-primary/20 transition-all duration-500 hover:shadow-[0_0_50px_rgba(212,255,0,0.05)] relative"
    >
...
export function HeroProductCard({ id, title, description, price, features, imageUrl }: ProductProps) {
  ...
  return (
    <motion.div 
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      className="group bg-[#09090B] border border-white/5 rounded-[3.5rem] overflow-hidden flex flex-col lg:flex-row min-h-[500px] hover:border-primary/20 transition-all duration-700 hover:shadow-[0_0_80px_rgba(212,255,0,0.05)] relative"
    >
      <div className="flex-1 p-10 lg:p-16 flex flex-col justify-center relative z-10">
        ...
        <Link href={`/product/${id}`}>
          <h2 className="text-4xl lg:text-6xl font-black text-white italic uppercase tracking-tighter mb-6 group-hover:text-primary transition-colors cursor-pointer leading-[0.9]">{title}</h2>
        </Link>
        ...
        <div className="flex flex-wrap items-center gap-6">
          <button 
            onClick={handleShopNow}
            className="bg-primary hover:bg-primary-hover text-black px-10 py-5 rounded-2xl font-black text-lg transition-all shadow-[0_0_30px_rgba(212,255,0,0.2)] hover:shadow-[0_0_50px_rgba(212,255,0,0.4)] hover:-translate-y-1 uppercase tracking-widest italic"
          >
            Acquire Now — ₹{price}
          </button>
          <button 
            onClick={handleAddToCart}
            className="px-8 py-5 rounded-2xl font-black text-xs text-white border border-white/10 hover:bg-white/5 transition-all uppercase tracking-[0.2em]"
          >
            Stage to Vault
          </button>
        </div>
      ...
      <Link href={`/product/${id}`} className="lg:w-1/2 relative min-h-[400px] lg:min-h-full overflow-hidden cursor-pointer">
        <img 
          src={imageUrl} 
          alt={title} 
