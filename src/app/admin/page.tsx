  ...
  return (
    <div className="animate-in fade-in duration-300">
      <div className="flex justify-between items-center mb-10">
        <div>
          <h1 className="text-3xl font-black text-zinc-900 dark:text-white italic uppercase tracking-tighter">Order <span className="text-primary neon-text">Analytics</span></h1>
          <p className="text-zinc-500 mt-2 font-medium">Real-time tracking of platform acquisitions and distributions.</p>
        </div>
        <div className="flex gap-6">
          <div className="bg-card px-6 py-3 rounded-2xl border border-zinc-200 dark:border-white/5 shadow-lg flex items-center gap-4 text-xs font-black uppercase tracking-widest">
            <span className="w-2.5 h-2.5 rounded-full bg-primary animate-pulse shadow-[0_0_8px_rgba(197,165,114,0.8)]"></span>
            <span className="text-zinc-500 dark:text-zinc-400">Inbound:</span>
            <span className="text-zinc-900 dark:text-white">{orders.filter(o => o.status === 'pending').length}</span>
          </div>
          <div className="bg-card px-6 py-3 rounded-2xl border border-zinc-200 dark:border-white/5 shadow-lg flex items-center gap-4 text-xs font-black uppercase tracking-widest">
            ...
            <span className="text-zinc-900 dark:text-white">{orders.filter(o => o.status === 'accepted').length}</span>
          </div>
        </div>
      </div>

      <div className="bg-card rounded-[2.5rem] shadow-2xl border border-zinc-200 dark:border-white/5 overflow-hidden">
            ...
            <tbody className="divide-y divide-zinc-200 dark:divide-white/5">
              {orders.map((order) => (
                <tr key={order.id} className="hover:bg-zinc-50 dark:hover:bg-white/5 transition-colors group">
                  <td className="p-8">
                    <div className="font-mono text-zinc-900 dark:text-white font-black italic tracking-tighter text-lg">{order.id}</div>
                    <div 
                      className="text-zinc-500 dark:text-zinc-600 text-[10px] font-bold uppercase tracking-widest mt-1 flex items-center gap-2"
                  ...
                  <td className="p-8">
                    <div className="font-black text-zinc-900 dark:text-white uppercase tracking-wider text-sm">{order.customer}</div>
                    <div className="text-zinc-500 text-xs mt-1 font-medium">{order.email}</div>
                    <div className="text-zinc-400 dark:text-zinc-600 text-[10px] mt-1 font-mono uppercase tracking-widest">{order.phone}</div>
                  </td>
                  <td className="p-8">
                    <span className="text-zinc-600 dark:text-zinc-400 font-bold uppercase tracking-widest text-xs border border-zinc-200 dark:border-white/10 px-3 py-1.5 rounded-lg bg-zinc-100 dark:bg-black/20">
                      {order.product}
                    </span>
                  </td>
                  <td className="p-8">
                    <span className="font-mono text-xl text-zinc-900 dark:text-white font-black italic tracking-tighter">₹{order.amount}</span>
                  </td>
                  <td className="p-8">
                  ...
                  <td className="p-8 text-right">
                    <div className="flex items-center justify-end gap-3">
                      <button className="p-3 text-zinc-500 hover:text-primary bg-zinc-100 dark:bg-white/5 hover:bg-primary/10 rounded-xl transition-all border border-transparent hover:border-primary/30" title="Inspect">
                      ...
                      {order.status === 'pending' && (
                        <>
                          <button 
                            ...
                            className="p-3 text-zinc-500 hover:text-primary bg-zinc-100 dark:bg-white/5 hover:bg-primary/10 rounded-xl transition-all border border-transparent hover:border-primary/30" title="Authorize Deployment">
                            <Check className="w-5 h-5" />
                          </button>
                          <button 
                            onClick={() => updateStatus(order.id, 'rejected')}
                            className="p-3 text-zinc-500 hover:text-red-500 bg-zinc-100 dark:bg-white/5 hover:bg-red-500/10 rounded-xl transition-all border border-transparent hover:border-red-500/30" title="Deny Entry">
                            <X className="w-5 h-5" />
                          </button>
                        </>
                      )}
